    ...
    fn process_tls_records_common<'c, 'i, T>(
        ...
        let state = 'outer: loop {
            ...
            if let Some(chunk) = self
                ...
                .pop()
            {
                break 'outer AppDataAvailable::new(self, incoming_tls, chunk).into();
            }
            ...
            if let Some(chunk) = self
                ...
                .pop()
            {
                break 'outer MustEncodeTlsData::new(self, chunk).into();
            }

            if let Some(msg) = self.core.deframe(&mut buffer)? {
                let mut state =
            ...
            {
                break ConnectionState::ConnectionClosed;
            } else if self
                .core
                .common_state
                .may_send_application_data
            {
                break ConnectionState::TrafficTransit(MayEncryptAppData { conn: self });
            } else {
                break ConnectionState::NeedsMoreTlsData { num_bytes: None };
            }
        };
...
pub struct UnbufferedStatus<'c, 'i, Data> {
    ...
    /// [`UnbufferedConnectionCommon::process_tls_records`] is called
    pub discard: usize,
